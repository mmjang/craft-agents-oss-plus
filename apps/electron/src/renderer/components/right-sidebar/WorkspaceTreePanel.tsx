/**
 * WorkspaceTreePanel - Displays files in the workspace directory as a tree view
 *
 * Features:
...
export function WorkspaceTreePanel({ workspaceId, closeButton, className }: WorkspaceTreePanelProps) {
  ...
  useEffect(() => {
    if (workspaceId) {
      const saved = storage.get<string[]>(storage.KEYS.workspaceTreeExpandedFolders, [], workspaceId)
  ...
  const saveExpandedPaths = useCallback((paths: Set<string>) => {
    if (workspaceId) {
      storage.set(storage.KEYS.workspaceTreeExpandedFolders, Array.from(paths), workspaceId)
    }
  }, [workspaceId])
  ...
  useEffect(() => {
    ...
    if (workspaceId) {
      ...
      window.electronAPI.watchWorkspaceFiles(workspaceId)
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden pb-2 min-h-0">
        {!workspaceId ? (
          <div className="px-4 text-muted-foreground select-none">
            <p className="text-xs">{t('workspace.empty', 'No workspace selected')}</p>
          </div>
        ) : files.length === 0 ? (
          <div className="px-4 text-muted-foreground select-none">
            <p className="text-xs">
              {isLoading
                ? t('workspaceTree.loading', 'Loading...')
                : t('workspaceTree.empty', 'No files in workspace.')}
            </p>
          </div>
        ) : (
